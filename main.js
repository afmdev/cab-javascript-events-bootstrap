...
function cardTemplate(data) {
  //create function that return the html
  // ! if you destructure then use the elements directly
  ...
  return `
    <div class="col-xs-4 col-sm-6 col-md-6 col-lg-6 col-xl-4 col-xxl-4 mb-4">
        <div class="card" id="${id}">
            ...
            onclick="insertModalDOM(getState(), ${id})">
                <span class="badge rounded-pill bg-dark views"><i class="fa fa-eye"> </i> ${views}</span>
                <span class="badge rounded-pill bg-dark likes"><i class="fa fa-heart"> </i> ${likes}</span>
                <img class="card-img-top" src="${webformatURL}" alt="${user}">
...
function modalTemplate(data) {
  //create function that return the html
  // ! same thing here, if you destructure use the destructured variables
...
function rightbarTemplate(data) {
  //create function that return the html
  const { id, user, tags, webformatURL, userImageURL } = data;
  return `
...
function handleSelectType(e) {
  ...
  const filteredItems = data.filter(
    (item) =>
      //   ! you can write it like this (same for select author filter)
      value.toLowerCase() === "all" ||
...
// ! why do you show clean filter only when selecting a color?
function checkLabel() {
  let colorSelected = document.getElementsByName("color");
...
let mostPopularItem = async () => {
  ...
  let imagesPerPages = 100;

  // ! here you could have divided the function into 2 functions: one to get the data from fetch, one to build the cards.
  // ! it doesn't make too much sense to first use setState() and then getState() if you are always in the same function
  ...
  let likes = images
    ...
    .map((item) => rightbarTemplate(item))
    .join("");
...
// ! very long and complex function, maybe you could simplify it
let searchImage = async () => {
  ...
  let valueOrientation = document.querySelector(
    'input[name="flexRadioDefault"]:checked'
  ).value;
  ...
  if (
    input != copyInput ||
    valueTypeImage != copyValueTypeImage ||
    valueOrientation != copyValueOrientation ||
    valueCategory != copyValueCategory
  ) {
    currentPage = 1;
  }
  ...
  let url = auth + query;
  // ! try catch and separate this function is too long
  let response = await fetch(url);
  let data = await response.json();
  setState(data);
  ...
  let pagPrev =
    currentPage === 1
      ? `<li class="page-item disabled"><a class="page-link" href="#prev" onclick="pagPrev()"><< Prev</a></li>`
      : `<li class="page-item"><a class="page-link" href="#prev" onclick="pagPrev()"><< Prev</a></li>`;

  let pagNext =
    currentPage === totalPages
      ? `<li class="page-item disabled"><a class="page-link" href="#prev" onclick="pagNext()">Next >></a></li>`
      : `<li class="page-item"><a class="page-link" href="#prev" onclick="pagNext()">Next >></a></li>`;
...
//   }
// };
// ! you could simply do it like that (same idea for pagNext())
const pagPrev = () => {
  if (currentPage !== 0) {
    currentPage--;
    searchImage();
